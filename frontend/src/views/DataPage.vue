<template>
  ...
  <div class="input-panel">
    ...
    <label>Regularization Strength (α): {{ alpha }}</label>
    <input type="range" min="0" max="10" step="0.1" v-model="alpha" />
    ...
    <select v-model="modelType">
      ...
      <option value="exponential">Exponential (a * exp(-b * x))</option>
      <option value="polynomial">Polynomial (a * x² + b * x + c)</option>
      <option value="logistic">Logistic (L / (1 + exp(-k(x - x₀))))</option>
      <option value="sigmoid">Sigmoid (1 / (1 + exp(-a(x - b))))</option>
      <option value="powerlaw">Powerlaw (a + xᵇ)</option>
    </select>

    <label>Regularization</label>
    <select v-model="regularization">
    ...
    <div class="param-heading" v-if="result">
      <h3>Estimated Parameters:</h3>
      <ul>
        <li v-for="(param, index) in result" :key="index">
          {{ paramLabels[index] || `Param ${index + 1}` }}: {{ param.toFixed(4) }}
        </li>
      </ul>
    </div>
  </div>
...
const paramLabels = computed(() => {
  switch (modelType.value) {
    ...
    case 'polynomial': return ['a', 'b', 'c']
    case 'logistic': return ['L', 'k', 'x₀']
    case 'sigmoid': return ['a', 'b']
    case 'powerlaw': return ['a', 'b']
    default: return []
  }
})
